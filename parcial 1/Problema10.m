...
while true
    ...
    % Mostrar menú
    disp("1. editar informacion");
    disp("2. agregar producto");
    disp("3. Eliminar producto");
    disp("4. planificacion");
    disp("5. lista de productos");
    disp("6. Salir");
        ...
        switch opcion
            case 1
              % Editar información de producto
                ...
                select_query = sprintf("SELECT * FROM produccion WHERE id = %d;", id_producto);
                ...
                if ~isempty(result.data)
                    ...
                    update_query = "UPDATE produccion SET cantidad = $1 WHERE id = $2;";
            ...
            case 2
                ...
                insert_query = "INSERT INTO produccion (nombre, cantidad) VALUES ($1, $2);";
            ...
            case 3
                ...
                if ~isempty(result.data)
                    ...
                    delete_query = "DELETE FROM produccion WHERE id = $1;";
    ...
    if ~isempty(result.data)
        ...
        disp("El producto no existe en la base de datos.");
            ...
            % Ver lista de productos10
                select_all_query = "SELECT * FROM produccion;";
